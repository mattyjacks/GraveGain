...
function LevelGenerator.GenerateSlot(sessionFolder, slotIndex, seed)
    ...
    elseif biome.name == "Mine" then
        for si = 1, 12 do
            ...
            PartBuilders.SpawnWall(slotModel, Vector3.new(0, beamY, 0), Vector3.new(GameData.LEVEL_WIDTH + wallThick, 3, 3), woodCol, Enum.Material.Wood)
            PartBuilders.SpawnWall(slotModel, Vector3.new(0, beamY, 0), Vector3.new(3, 3, GameData.LEVEL_WIDTH + wallThick), woodCol, Enum.Material.Wood)
            PartBuilders.SpawnWall(slotModel, Vector3.new(-halfW + 2, beamY - 6, 0), Vector3.new(4, 12, 4), woodCol, Enum.Material.Wood)
            PartBuilders.SpawnWall(slotModel, Vector3.new(halfW - 2, beamY - 6, 0), Vector3.new(4, 12, 4), woodCol, Enum.Material.Wood)
            
            if rng:NextNumber() < 0.6 then
                local lX = rng:NextNumber(-halfW/2, halfW/2)
                local lZ = rng:NextNumber(-halfW/2, halfW/2)
                PartBuilders.SpawnGlowBlock(slotModel, Vector3.new(lX, beamY - 3, lZ), Vector3.new(2, 3, 2), Color3.fromRGB(220, 170, 60), 2.5, 18)
            end
        end
    end
