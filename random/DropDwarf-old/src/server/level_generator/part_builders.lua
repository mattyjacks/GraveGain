...
function PartBuilders.MakePart(parent, name, pos, size, color, material, transparency)
    ...
    p.CastShadow    = true
    -- Physics stability: prevent bounce/pogo stick behavior
...
function PartBuilders.SpawnPlatform(parent, pos, size, color, material, terrainType, biome)
    ...
    -- Terrain-specific physics: soft = more friction, hard = less bouncy
    local physProps = PhysicalProperties.new(0.7, 0.3, 0, 1, 1)
    if terrainType == PartBuilders.TERRAIN_SOFT then
        physProps = PhysicalProperties.new(0.9, 0.5, 0, 1, 1) -- high friction
    elseif terrainType == PartBuilders.TERRAIN_HARD then
        physProps = PhysicalProperties.new(0.5, 0.2, 0, 1, 1) -- dense, low bounce
    end
    ...
    main.CustomPhysicalProperties = physProps
