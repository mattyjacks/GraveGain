...
local function makeBlock(parent, name, pos, size, color, material)
    ...
    -- Physics stability: prevent bounce/pogo behavior on hub surfaces
...
function HubGenerator.Generate(workspace)
    ...
    -- Solid collision floor slab - single part, no seams, no gaps.
    -- Decorative tiles above it have CanCollide=false so only this slab handles physics.
    local floorSlab = makeBlockColor3(hubFolder, "FloorSlab",
        Vector3.new(0, HUB_Y - 0.5, 0),
        Vector3.new(HUB_R * 2 + 8, 1, HUB_R * 2 + 8),
        Color3.fromRGB(75, 70, 65), Enum.Material.SmoothPlastic)
    floorSlab.CanCollide = true
    -- Critical: zero elasticity to prevent pogo stick bounce
    floorSlab.CustomPhysicalProperties = PhysicalProperties.new(0.7, 0.3, 0, 1, 1)

    -- Decorative checkerboard tiles on top (no collision - purely visual)
    local BLOCK = 4
    for x = -HUB_R, HUB_R, BLOCK do
        for z = -HUB_R, HUB_R, BLOCK do
            ...
            if dist <= HUB_R then
                ...
                local tile = makeBlockColor3(hubFolder, "Floor", Vector3.new(x, HUB_Y - 0.45, z),
                    Vector3.new(BLOCK, 0.1, BLOCK), col, Enum.Material.SmoothPlastic)
    ...
    -- Spawn area label (purely visual, flush with floor, no collision)
    local spawnLabel = makeBlockColor3(hubFolder, "SpawnLabel",
        Vector3.new(0, HUB_Y - 0.45, 0),
        Vector3.new(12, 0.1, 12),
        Color3.fromRGB(80, 200, 80), Enum.Material.Neon)
    spawnLabel.CanCollide = false
    spawnLabel.Transparency = 0.5
    addLabel(spawnLabel, "SPAWN", 24, 3)

    -- Ensure SpawnLocation is non-collidable and invisible.
    -- Position is owned by default.project.json (Y=603, 3 studs above floor at Y=600).
    -- Do NOT reposition it here - moving it into/below the floor causes spawn ejection.
