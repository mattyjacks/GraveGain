...
-- Force-disable collision on any SpawnPad or SpawnLocation that hub_generator created.
-- These collidable surfaces at hub floor level cause physics ejection when the server
-- teleports the HRP into them. They are decorative only; the floor tiles are walkable.
for _, desc in ipairs(workspace:GetDescendants()) do
    if desc:IsA("BasePart") and desc.Name == "SpawnPad" then
...
local function applyStats(player, stats)
    ...
    -- Disable the Jumping state entirely so Roblox's PlayerModule can't launch the character.
    -- movement.lua uses AssemblyLinearVelocity directly for air jumps instead.
    humanoid.JumpHeight = 0
    humanoid.JumpPower  = 0
    humanoid:SetStateEnabled(Enum.HumanoidStateType.Jumping, false)
    -- NOTE: HipHeight is set once in CharacterAdded, not here (avoid physics recalculation)
end

-- One-time character setup (HipHeight, physics properties)
local function setupCharacterPhysics(character)
    local humanoid = character:FindFirstChildOfClass("Humanoid")
    if not humanoid then return end
    -- Set HipHeight once on spawn (prevents pogo stick physics)
    if humanoid.RigType == Enum.HumanoidRigType.R15 then
        humanoid.HipHeight = 2.0
    end
    -- Zero elasticity on all parts to prevent character bouncing
    for _, part in ipairs(character:GetDescendants()) do
        if part:IsA("BasePart") then
            part.CustomPhysicalProperties = PhysicalProperties.new(0.7, 0.3, 0, 1, 1)
        end
    end
end
...
-- Request to start a level with a given seed
Networking.OnServer(Networking.Events.RequestStartLevel, function(player, seed)
    local state = getState(player)
    if not state or state.inLevel then return end
    local SessionHandler = require(script.Parent.handlers.session_handler)
    SessionHandler.StartLevel(player, seed, getState)
end)
...
Players.PlayerAdded:Connect(function(player)
    ...
    player.CharacterAdded:Connect(function(character)
        ...
        -- Immediately disable jumping so character can't auto-jump or get flung
        local humanoid = character:FindFirstChildOfClass("Humanoid")
            or character:WaitForChild("Humanoid", 5)
        local hrpImmediate = character:FindFirstChild("HumanoidRootPart")
        ...
        -- One-time physics setup: HipHeight + zero elasticity (prevents pogo)
        setupCharacterPhysics(character)
        -- Handle parts that load after (accessories etc)
        character.DescendantAdded:Connect(function(desc)
            if desc:IsA("BasePart") then
                desc.CustomPhysicalProperties = PhysicalProperties.new(0.7, 0.3, 0, 1, 1)
            end
        end)

        -- NOTE: All anti-pogo is handled by zero-elasticity parts (character + ground)
        -- No velocity manipulation needed - let physics work naturally

        -- Teleport to hub spawn after Humanoid is ready
        -- Spawn at Y=608 (8 studs above floor) to prevent feet clipping
        if not state or not state.inLevel then
            task.spawn(function()
                local hrp = character:WaitForChild("HumanoidRootPart", 2)
                local humanoid = character:WaitForChild("Humanoid", 2)
                if hrp and humanoid then
                    task.wait(0.1) -- Small delay for physics to settle
                    hrp.CFrame = CFrame.new(0, GameData.HUB_FLOOR_Y + 8, 0)
                end
            end)
        end
