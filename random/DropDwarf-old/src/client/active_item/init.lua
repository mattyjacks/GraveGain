...
function ActiveItem:_checkSpring()
    ...
    local now = tick()
    if now - self.lastSpringTime < 1.2 then return end  -- longer cooldown prevents repeated flings

    -- Only trigger if we were actually falling onto the pad (not just walking onto it)
    local impact = math.abs(self.lastVelY)
    if impact < 8 then return end  -- must be falling at least 8 studs/s to trigger
