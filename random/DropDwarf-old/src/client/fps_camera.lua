...
local function hideCharacter(char)
    ...
    for _, obj in ipairs(char:GetDescendants()) do
        -- Skip flashlight parts so the beam and spotlight keep working
...
function FPSCamera:Start(dropType)
    ...
    if self.dropType == "fps" then
        ...
        -- Immediately position camera at character so it doesn't start at origin
        local hrp = self:GetHRP()
        if hrp then
            local eyePos = hrp.Position + Vector3.new(0, 1.5, 0)
            self.camera.CFrame = CFrame.new(eyePos) * CFrame.Angles(0, self.yaw, 0) * CFrame.Angles(self.pitch, 0, 0)
        end
...
function FPSCamera:UpdateFPS()
    ...
    -- Use HRP + eye offset instead of head.Position (head can desync from body)
    local EYE_OFFSET = Vector3.new(0, 1.5, 0)
    local eyePos = hrp.Position + EYE_OFFSET
