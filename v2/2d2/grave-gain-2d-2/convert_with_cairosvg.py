#!/usr/bin/env python3
"""
Convert SVG emoji to PNG at 256x256 using cairosvg or PIL
"""
...
def emoji_to_hex(emoji: str) -> str:
    """Convert emoji to hex codepoint string, excluding variation selectors"""
    hex_parts = []
    for char in emoji:
        cp = ord(char)
        # Skip variation selectors and zero-width joiners
        if cp > 0 and cp not in [0xFE0F, 0x200D, 0x200C]:
            hex_parts.append(f"{cp:x}")
    return "-".join(hex_parts)

def find_svg_file(emoji: str, svg_dir: Path) -> Path:
    """Find the SVG file for an emoji"""
    ...
    # Try with padding
    padded = hex_code.split('-')[0].zfill(4)
    if len(hex_code.split('-')) > 1:
        padded = "-".join([p.zfill(4) for p in hex_code.split('-')])
    
    svg_file = svg_dir / f"{padded}.svg"
    if svg_file.exists():
        return svg_file
    
    return None

def convert_with_cairosvg(svg_path: str, png_path: str, size: int = 256) -> bool:
...
def convert_with_pil(svg_path: str, png_path: str, size: int = 256) -> bool:
...
def main():
    ...
    # All emoji from game data
    emoji_data = [
        # Races
        ("\U0001F469\u200D\U0001F680", "human"),
        ("\U0001F9DD\u200D\u2640\uFE0F", "elf"),
        ("\u26CF\uFE0F", "dwarf"),
        ("\U0001F479", "orc"),
    ...
    print("\n=== Converting Emoji to PNG (256x256) ===\n")
    print(f"Total emoji to convert: {len(emoji_data)}\n")
    ...
    for emoji, name in emoji_data:
        svg_file = find_svg_file(emoji, svg_dir)
        
        if not svg_file:
            hex_code = emoji_to_hex(emoji)
            print(f"✗ {emoji} ({name}) - SVG not found ({hex_code})")
            not_found += 1
            continue
        ...
        # Try cairosvg first
        if convert_with_cairosvg(str(svg_file), str(png_file), 256):
            print(f"✓ {emoji} ({name})")
            converted += 1
        # Try PIL/Wand
        elif convert_with_pil(str(svg_file), str(png_file), 256):
            print(f"✓ {emoji} ({name})")
            converted += 1
        else:
    ...
    print(f"Total: {converted + failed + not_found}\n")
